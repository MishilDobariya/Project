...
function F_getUserTests()
{
    ...
    if ($r = F_db_query($sql, $db)) {
		...
		//-----------------------score--------------------------------					
							...
							mysqli_select_db($link,"tcexam" );
							$select = mysqli_query($link,"SELECT testuser_user_id FROM tce_tests_users WHERE testuser_id='".$testuser_id."'");
							
							while ($row = mysqli_fetch_array($select)) {
								$id = $row['testuser_user_id'];
							}
							$select = mysqli_query($link,"SELECT user_name FROM tce_users WHERE user_id='".$id."'");
							while ($row = mysqli_fetch_array($select)) {
								$name = $row['user_name'];
							}
							
							mysqli_select_db($link,"karyarthin" );
							mysqli_query($link, "UPDATE `applicant` SET `Score` = '".$usrtestdata['user_score']."' WHERE `applicant`.`Name` = '".$name. "' and `applicant`.`Post` ='".$m['test_name']."'" );
							mysqli_close($link);
